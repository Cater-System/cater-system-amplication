...
import { Prisma, Order, Product, Coupon, Delivery, User } from "@prisma/client";
...
export class OrderServiceBase {
  ...
  async getCoupon(parentId: string): Promise<Coupon | null> {
    return this.prisma.order
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getDelivery(parentId: string): Promise<Delivery | null> {
    return this.prisma.order
      .findUnique({
        where: { id: parentId },
      })
